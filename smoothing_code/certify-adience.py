...
from model_schema2 import MLP1, DeepCNN, AdienceClassifier
...
if __name__ == "__main__":
    ...
    ORDERED_CLASS_LABELS = ['0-2', '4-6', '8-13', '15-20', '25-32', '38-43', '48-53', '60-']
    # checkpoint = torch.load(args.base_classifier, map_location=device)
    # base_classifier = get_architecture(checkpoint["arch"], args.dataset)
    # base_classifier.load_state_dict(checkpoint['state_dict'])
    
    if 'mlp' in args.model_name.lower():
        base_classifier = MLP1(150528, len(ORDERED_CLASS_LABELS))
    elif 'adience_classifier' in args.model_name.lower():
        base_classifier = AdienceClassifier(len(ORDERED_CLASS_LABELS))
    ...
    print("idx\tlabel\tgender\tbase\tsmooth\tradius\ttime", file=f, flush=True)
    ...
    for i in range(len(dataset)):
        ...
        if args.split == 'train':
            gender_idx = dataset.ds.train_image_genders[img_idx]
        else:
            gender_idx = dataset.ds.test_image_genders[img_idx]
        ...
        print("{}\t{}\t{}\t{}\t{}\t{:.3}\t{}".format(
            i, label, dataset.ds.gender_id_to_label[gender_idx], base_prediction, 
