...
from custom_data_loaders import Adience, UTKFace, CIFAR100Super
...
DATASETS = ["imagenet", "imagenet32", "cifar10", "cifar100", "cifar100super", "adience", "utkface"]
...
def get_dataset(dataset: str, split: str) -> Dataset:
    ...
    if dataset == "imagenet":
        ...
        return _cifar100super(split)
    
    elif dataset == "adience":
        return _adience(split)
    
    elif dataset == "utkface":
        return _utkface(split)
...
def get_num_classes(dataset: str):
    ...
    if dataset == "imagenet":
        ...
        return 10
    elif dataset == "cifar100" or dataset == "cifar100super":
        return 100
    elif dataset == "utkface":
        return 5
    elif dataset == "adience":
...
data_transform_cifar10 = transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Normalize(_CIFAR10_MEAN, _CIFAR10_STDDEV)
                    ])
data_transform_cifar100 = transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Normalize(_CIFAR100_MEAN, _CIFAR100_STDDEV)
                    ])

def _adience(split: str) -> Dataset:
    ds_obj = Adience('adience')
    dataset = PytorchLoader(ds=ds_obj, portion=split)
    return dataset
    
def _utkface(split: str) -> Dataset:
    ds_obj = UTKFace('utkface')
    dataset = PytorchLoader(ds=ds_obj, portion=split)
    return dataset
...
def _cifar10(split: str) -> Dataset:
    ...
    testset = datasets.CIFAR10(root=dataset_path, train=False,
                               download=True, transform=data_transform_cifar10)
    return testset
...
def _cifar100(split: str) -> Dataset:
    ...
    testset = datasets.CIFAR100(root=dataset_path, train=False,
                               download=True, transform=data_transform_cifar100)
    return testset

def _cifar100super(split: str) -> Dataset:
    ds_obj = CIFAR100Super('CIFAR100super')
    dataset = PytorchLoader(ds=ds_obj, portion=split)
    return dataset
